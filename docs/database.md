...
- [Overview](#model-overview)
  ...
  - [Resources](#model-resources)
    - [account_types](#model-account-types)
    - [users](#model-users)
    - [services](#model-services)
    - [tokens](#model-tokens)
    - [team_members](#model-team-members)
    - [training_series](#model-training-series)
...
The logged-in end user that performs all operations within the app's front end. Other resources will be retrieved based on the currently logged-in user's email having been set on res.locals in authentication, and will not be able to retrieve information in the database they did not create.
...
Team Members that will receive messages from the admin User after having been assigned to a Training Series with Messages. Can have other Team Members assigned as their manager or mentor, which allows for other options for who gets sent messages when the Team Member is assigned to a Training Series.
...
These sections describe the standards adhered when accessing the back end, in the interest of being scalable, easy to work with once you know the codebase, integrate extra table resources, extra endpoints to access those resources, or to audit any of the existing methods and endpoints.

### Methods

Originally almost all of the existing resources could had anywhere between 5 and 10 database methods, many of which were superfluous. All resources now have 4 standardized methods to minimize confusion and simplify both bug-hunting and expansion of functionality. The few exceptions here are that Responses do not require an .update() method in the current scope of the app, Tokens.add() to first perform a nested Services.find() to retrieve the desired services.id, Users.find() selects different fields depending on whether it was passed a filter or not, and Notifications.update() does not return the updated resource as it is not currently required.
...
Methods use a standardized abbreviations for methods that require filtering or joining onto other resource tables: messages AS m, users AS u, training_series AS ts, team_members AS tm, responses AS r, tokens AS tk, services AS s, account_types AS a. See documentation for [Knex.js](https://knexjs.org/#Builder-join) for further information on joins and the syntax used.
